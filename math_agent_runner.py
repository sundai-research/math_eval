...
async def solve_math_problem_async(
    ...
    async def create_and_write_result(success: bool, iterations: int, status: str, conversation: List[Dict], tool_attempts: int = 0, final_answer: str = "", extracted_answer: str = ""):
        result = {
            "success": success,
    ...
    async with semaphore:
        ...
        if not os.environ.get("GROQ_API_KEY"):
            if verbose:
                print(f"Problem {unique_id}-{sample_idx}: GROQ_API_KEY not set")
            return await return_result(False, "GROQ_API_KEY not set")
        ...
        while iteration < max_iterations:
            ...
            if verbose:
                print(f"Problem {unique_id}-{sample_idx}: Iteration {iteration}/{max_iterations}")
            ...
            try:
                ...
                if verbose:
                    print(f"Problem {unique_id}-{sample_idx}: ⏰ Timeout after {elapsed_time:.1f}s")
                    print(f"  💥 Last exception: {str(e.last_attempt.exception())}")
                    import traceback
                    print(f"  📍 Traceback: {traceback.format_exc()}")
                return await return_result(False, f"Retry timeout after {elapsed_time:.1f} seconds - {str(e.last_attempt.exception())}")
            except Exception as e:
                if verbose:
                    print(f"Problem {unique_id}-{sample_idx}: 💥 Unexpected API error: {e}")
                    import traceback
                    print(f"  📍 Traceback: {traceback.format_exc()}")
            ...
            if verbose:
                ...
                if assistant_message.content:
                    ...
                    print(f"Problem {unique_id}-{sample_idx}: 📄 Model response preview: {content_preview}...")
                    ...
                    if "\\boxed{" in content:
                        ...
                        if boxed_matches:
                            print(f"  📦 Found \\boxed{{}} content in text: {boxed_matches}")
                    ...
                    if any(phrase in content.lower() for phrase in ["the answer is", "therefore", "thus", "so the", "final answer"]):
                    ...
                    print(f"Problem {unique_id}-{sample_idx}: 📄 Model response: [No content]")
                
                if assistant_message.tool_calls:
                    print(f"Problem {unique_id}-{sample_idx}: 🔧 Model made {len(assistant_message.tool_calls)} tool call(s)")
                    ...
                    for i, tc in enumerate(assistant_message.tool_calls):
                        ...
                        print(f"     Args: {args_preview}{'...' if len(tc.function.arguments) > 200 else ''}")
                        
                        # Check for patterns in tool arguments
                        if "import" in tc.function.arguments or "print" in tc.function.arguments:
                    ...
                    print(f"Problem {unique_id}-{sample_idx}: ❗ Model made NO tool calls")
                    if iteration == 1:
            ...
            if assistant_message.tool_calls:
                if verbose:
                    print(f"Problem {unique_id}-{sample_idx}: Submitting answer via tool")
                ...
                for tool_call in assistant_message.tool_calls:
                    ...
                    try:
                        ...
                        if verbose:
                            print(f"Problem {unique_id}-{sample_idx}: 💥 Failed to parse tool arguments: {e}")
                            print(f"  📄 Raw arguments: {tool_call.function.arguments}")
                        tool_result = f"❌ ERROR: Failed to parse tool arguments: {str(e)}"
                        function_args = {}
                        last_submitted_answer = ""
                    else:
                        ...
                        if verbose:
                            print(f"Problem {unique_id}-{sample_idx}: 🔧 Calling tool '{function_name}'")
                        
                        # Execute the tool
                        tool_result = execute_math_tool_call(function_name, function_args, ground_truth, verbose)
                    ...
                    if boxed_content:
                        ...
                        if len(last_extracted_answer) > 2 and last_extracted_answer[0] == '"' and last_extracted_answer[-1] == '"':
                    ...
                    if "CORRECT!" in tool_result:
                        if verbose:
                            print(f"Problem {unique_id}-{sample_idx}: Solved in {iteration} iterations")
        ...
        if verbose:
            print(f"Problem {unique_id}-{sample_idx}: ❌ Failed after {max_iterations} iterations")
            print(f"  📊 Total tool attempts made: {tool_call_count}")
            print(f"  💬 Final conversation length: {len(conversation_messages)} messages")
            ...
            if boxed_in_text_count > 0:
                failure_analysis.append(f"Put \\boxed{{}} in text {boxed_in_text_count} times")
            ...
            if last_submitted_answer:
                print(f"  📝 Last submitted: '{last_submitted_answer[:100]}{'...' if len(last_submitted_answer) > 100 else ''}'")
                print(f"  🎯 Expected: '{ground_truth}'")
        
        return await return_result(False, f"Reached maximum iterations ({max_iterations}) without solving")
