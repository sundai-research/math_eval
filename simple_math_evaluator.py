...
async def run_evaluation(
    ...
    for problem in selected:
        for sample_idx in range(samples):
            ...
            task = process_sample(
                client, problem, sample_idx, 
                is_chinese, writer, semaphore, temperature
            )
            tasks.append(task)
...
def evaluate(
    ...
    results = asyncio.run(run_evaluation(min_idx, max_idx, samples, temperature, concurrency, dataset, output))
...
def evaluate_jsonl_results(results: List[Dict], dataset: str) -> Dict:
    ...
    for unique_id, samples in problems.items():
        ...
        for sample in samples:
            ...
            response_length = sample.get("response_length", len(sample.get("response", "")))
            ...
            if is_correct:
                ...
                metrics["by_subject"][subject]["correct_output_tokens"].append(output_tokens)
                metrics["by_subject"][subject]["correct_response_lengths"].append(response_length)
            else:
                ...
                metrics["by_subject"][subject]["incorrect_output_tokens"].append(output_tokens)
                metrics["by_subject"][subject]["incorrect_response_lengths"].append(response_length)
    ...
    cons_at_sample = (
        ...
        if metrics["consistency_total"] > 0 else 0
    )
    ...
    for subject, data in metrics["by_subject"].items():
        ...
        subject_cons_at_sample = (
            ...
            if data["consistency_total"] > 0 else 0
        )
        avg_output_tokens = np.mean(data["output_tokens"]) if data["output_tokens"] else 0
        avg_response_length = np.mean(data["response_lengths"]) if data["response_lengths"] else 0
        avg_correct_tokens = np.mean(data["correct_output_tokens"]) if data["correct_output_tokens"] else 0
        avg_correct_length = np.mean(data["correct_response_lengths"]) if data["correct_response_lengths"] else 0
        avg_incorrect_tokens = np.mean(data["incorrect_output_tokens"]) if data["incorrect_output_tokens"] else 0
        avg_incorrect_length = np.mean(data["incorrect_response_lengths"]) if data["incorrect_response_lengths"] else 0
    ...
    # Overall metrics
    total_avg_output_tokens = np.mean(metrics["total"]["output_tokens"]) if metrics["total"]["output_tokens"] else 0
    total_avg_response_length = np.mean(metrics["total"]["response_lengths"]) if metrics["total"]["response_lengths"] else 0
    correct_avg_tokens = np.mean(metrics["total"]["correct_output_tokens"]) if metrics["total"]["correct_output_tokens"] else 0
    correct_avg_length = np.mean(metrics["total"]["correct_response_lengths"]) if metrics["total"]["correct_response_lengths"] else 0
    incorrect_avg_tokens = np.mean(metrics["total"]["incorrect_output_tokens"]) if metrics["total"]["incorrect_output_tokens"] else 0
    incorrect_avg_length = np.mean(metrics["total"]["incorrect_response_lengths"]) if metrics["total"]["incorrect_response_lengths"] else 0
...
def analyze(
    ...
    dataset: str = typer.Option("EN-EASY", "--dataset", help="Dataset type for language detection")
):
    ...
    for subject, metrics_data in final_metrics["by_subject"].items():
        ...
        print(f"    Avg correct response length: {metrics_data['avg_correct_length']:.2f}")
        print(f"    Avg incorrect output tokens: {metrics_data['avg_incorrect_tokens']:.2f}")
        print(f"    Avg incorrect response length: {metrics_data['avg_incorrect_length']:.2f}")
...
def evaluate_with_tools(
    ...
    results = asyncio.run(run_tool_evaluation(min_idx, max_idx, samples, temperature, concurrency, dataset, output, max_iterations))
