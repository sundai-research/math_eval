    ...
    try:
        ...
        if boxed_answers:
            ...
            if (
                ...
                and extracted_content[0] == '"'
                and extracted_content[-1] == '"'
            ):
                extracted_content = extracted_content[1:-1]

            if verbose and not boxed_answers:
                print(
                    f"  📝 Note: Answer submitted without \\boxed{{}}, using raw content"
                )
...
def execute_math_tool_call(
    ...
    # here now we need to serialize the output back into str
    try:
        result = json.dumps(result)
    except json.JSONDecodeError:
        # okay this does not work, so try to just cast into str
        result = str(result)
    return result
